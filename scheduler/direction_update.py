...
from avia_api.exceptions import (
    DatabaseUpdateTicketError,
    DatabaseUpdateDirectionSentPostsError,
    TicketApiConnectionError,
    TicketsAPIError,
    TicketsParsingError,
    MissingTicketsError, DatabaseGetTicketError, DatabaseAddTicketError, AddNewTicket
)
class DirectionUpdate:
    ...
    async def update(self):
        ...
        for direction in _directions:
            await update_direction(direction=direction, api=api, bot=self.bot, settings=settings)
        logger.info(f"Проверка всех направлений завершена ({len(_directions)})")


async def update_direction(direction: Direction, api: TicketsApi, bot: BotService, settings: PriceSettings):
        # Получение нового билета через API
        try:
            new_ticket = await get_tickets_api(api=api, direction=direction)
        except (MissingTicketsError, TicketsAPIError, TicketsParsingError, TicketApiConnectionError):
            return
...
async def get_ticket_db(direction: Direction, new_ticket: Ticket) -> Ticket | None:
    ...
    if not old_ticket:
        ...
        try:
            ...
            logger.info(f"Добавлен билет: {direction.direction_from} {direction.origin_code} - {direction.direction_to} {direction.destination_code} new_ticket.destination_code = {new_ticket.destination_code}")
            raise AddNewTicket()
...
async def checking_update(
    ...
    # Цена билета превышает указанный порог
        ...
        try:
            ...
            logger.info(f"Превышение максимальной цена, обновление БД {direction.id_direction} - {direction.destination_code} : {new_price} >= {direction.max_price}")
